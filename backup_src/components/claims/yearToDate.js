...
const YearToDatePage = () => {
    ...
    const formatMemberDDList = () => {
      ...
      customerInfo["dependents"].forEach(dependent => {
        var member = {label: formatNameCapitalize(dependent.firstName)+" "+formatNameCapitalize(dependent.lastName), value: dependent.memberId, planName: dependent.planName};
  ...
  return (
          !oopList || !currentOOP || loading ? <Spinner/> : currentOOP.oopLength ?
          <YearToDateContainer>
            <GlobalStyle />
            <DropDownWrapper>
            ...
            showImage={true}
                          selected={{label: formatNameCapitalize(customerInfo.firstName)+" "+formatNameCapitalize(customerInfo.lastName)}}
                          values={memberships}
                ...
                <Fragment key={oopItem.id}>
                  ...
                  <IndividualCnt>
                  {
                    oopItem.data.map(item => 
                      <Paper key={item.id}>
                        <HeaderTxt>
                          { cards[item.title].header }
                        </HeaderTxt>
                        <Content>
                          <Annualdeduct>
                              { cards[item.title].maxTitle }
                          </Annualdeduct>
                          <Amount>
                              { currentOOP.oop[item.title] !== undefined ? "$"+numberWithCommas(
                                parseFloat(currentOOP.oop[item.title].maximumamount).toFixed(2)) : "Not Available"
                              }
                          </Amount>
                        </Content>
                        <Content>
                            <Annualdeduct>
                              { cards[item.title].currrentTitle }
                            </Annualdeduct>
                            <Amount>
                                { currentOOP.oop[item.title] !== undefined ? "$"+numberWithCommas(
                                  parseFloat(currentOOP.oop[item.title].currentamount).toFixed(2)) : "Not Available"
                                }
                            </Amount>
                        </Content>
                      </Paper>
