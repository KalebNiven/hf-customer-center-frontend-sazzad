        ...
        <div>
            {
                data && getQuestionById(data.list, active, visited, hraMemberInfo).question_answers.map(answer => {
                    ...
                    return <InputCheckbox key={id} id={id} title={answer_text} code={answer_code} answer_with_text={answer_with_text} isSelected={isSelected} setIsSelected={setIsSelected} />
                })
            }
        </div>
    )
