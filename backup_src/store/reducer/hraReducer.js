...
const handleNextQuestion = (state, hraMemberInfo) => {
  ...
  const currentQuestion = getQuestionById(state.data.list, currentQuestionId, state.form.visited, hraMemberInfo);
  ...
  if(question_type === SINGLE_SELECT) {
    const selectedAnswerId = selected[currentQuestionId] && selected[currentQuestionId][0].id
    pending = [...state.form.pending];
    pending.unshift(...getAnswerById(question_answers, selectedAnswerId).answer_dependencies.map(item => item.dependency_uuid.question_id));
  }
  ...
  if(question_type === MULTY_SELECT) {
    ...
    selected[currentQuestionId].forEach(item => {
      const currentQ = getQuestionById(state.data.list, currentQuestionId, state.form.visited, hraMemberInfo);
      const currentQdeps = getAnswerById(currentQ.question_answers, item.id).answer_dependencies.reduce((items, item) => {
        if(!items.includes(item.dependency_uuid.question_id) && !pending.includes(item.dependency_uuid.question_id)) {
  ...
  if(question_type === MATRIX) {
    ...
    pending.unshift(...question_answers[0].answer_dependencies.map(item => item.dependency_uuid.question_id));
    ...
    for(let i = 0; i < matrix_questions.length; i++) {
      ...
      const selectedAnswerId = selected[matrixAnswerId] && selected[matrixAnswerId][0].id;
      pending.unshift(...getAnswerById(question_answers, selectedAnswerId).answer_dependencies.map(item => item.dependency_uuid.question_id));
    }
  }
...
const handlePreviousQuestion = (state, hraMemberInfo) => {
  ...
  const currentQuestion = getQuestionById(state.data.list, activeId, state.form.visited, hraMemberInfo);
  const { question_type, matrix_questions, question_answers } = currentQuestion;
  ...
  // select
  if(question_type === SINGLE_SELECT) {
    pending = pending.slice(getAnswerById(question_answers, selectedAnswer[0].id).answer_dependencies.length - 1);
  }
  ...
  if(question_type === MULTY_SELECT) {
    ...
    selectedAnswers.forEach(selected_answer => {
      const answer = question_answers.find(question_answer => selected_answer.id === question_answer.id)
      deps = [...deps, ...answer.answer_dependencies]
    })
    // remove duplicates from deps
    deps = deps.filter((item, index, self) => index === self.findIndex(t => t.dependency_uuid.question_id === item.dependency_uuid.question_id))
  ...
  if(question_type === MATRIX) {
    ...
    for(let i = 0; i < matrix_questions.length; i++) {
      const item = matrix_questions[i];
      const { question_answers, question_id } = item;
      const selectedAnswerId = selected[activeId + '-' + question_id] && selected[activeId + '-' + question_id][0].id;
      if(isFirst) {
        pending = pending.slice(getAnswerById(question_answers, selectedAnswerId).answer_dependencies.length - 1);
        isFirst = false;
      } else {
        pending = pending.slice(getAnswerById(question_answers, selectedAnswerId).answer_dependencies.length);
      }
    }
...
const generageAnswerList = ({ id, info, type }, state) => {
  // handle checkbox
  if(state.form.selected[id] && type === MULTY_SELECT || state.form.selected[id] && type === FREE_TEXT) {
    // checkbox click
    if(type === MULTY_SELECT){
      // uncheck (if it's already exist)
      if(state.form.selected[id].some(item => item.answer_code === info.answer_code)) {
        return [...state.form.selected[id].filter(item => item.answer_code !== info.answer_code)]
      } else {
        // check (if not exist)
    ...
    if(type === FREE_TEXT){
      ...
      answers.find(answer => {
        if(answer.answer_code === info.answer_code) { answer.answer_value = info.answer_value }
      });
      return answers
    }
  }
...
export default function hra(state = initialState, action) {
  switch (action.type) {
    ...
    case actionTypes.REQUEST_PREVIOUS_QUESTION: {
      const { visited, active, pending } = handlePreviousQuestion(state, action.payload.hraMemberInfo);
      return {
        ...state,
        form: {
          ...state.form,
    ...
    case actionTypes.SET_VISITED_QUESTION: {
      let currentQuestion = getQuestionById(state.data.list, state.form.active, state.form.visited, action.payload.userInfo);
    ...
    case actionTypes.RECEIVE_HRA_QUESTIONS: {
      return {
        ...
        data: {
          ...state.data,
          // list: action.payload,
          list: action.payload.hra_survey_questions.sort((a, b) => (a.question_id > b.question_id) ? 1 : -1),
