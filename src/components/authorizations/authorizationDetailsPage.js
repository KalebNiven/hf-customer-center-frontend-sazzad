...
const AuthorizationDetailsPage = () => {
  ...
  const authorizationDetails = useSelector((state) => state.authorizationDetails.authorizationDetails);
  const authorizationDetailsLoading = useSelector((state) => state.authorizationDetails.loading);
    ...
    <Container>
     ...
     > 
        ...
        <Breadcrumbs separator={<NavigationIcon src = "/react/images/icn-arrow-right.svg" />} >
          <BreadcrumbsAuthorization onClick = {() => history.push({
            pathname: "/authorizations"})}>Authorizations</BreadcrumbsAuthorization>
          <AuthorizationDetailsText>Authorization #: {authorizationDetails.authorizationHeader.authorizationId}</AuthorizationDetailsText>
      </Breadcrumbs>
      </BreadcrumbNavigation>
        <AuthorizationText>Authorization #: {authorizationDetails.authorizationHeader.authorizationId}</AuthorizationText> 
        <IndividualCnt>
            <LeftContainer>
        <Paper first>
            <Patient>
                ...
                <PatientName>
                  {authorizationDetails.patient.firstName.concat(' ',authorizationDetails.patient.lastName)}
                </PatientName>
                <MemberIdTxt>
                MEMBER ID
                </MemberIdTxt>
                <MemberId>
                {authorizationDetails.patient.id}
                </MemberId>
                <Section>
                <TypeTxt>
                TYPE
                </TypeTxt>
                <Type>{authorizationDetails.patient.type}
                </Type>
                </Section>
                <Section>
                 <StatusTxt>
                ...
                {authorizationDetails.authorizationHeader.authorizationStatus === "Approve" ? 
                <ProcessedStatus>
                    Approved
                </ProcessedStatus> : (authorizationDetails.authorizationHeader.authorizationStatus === "Pend") ?
                <PendingStatus>
                    Pending
                </PendingStatus> : (authorizationDetails.authorizationHeader.authorizationStatus === "Partial Deny" || authorizationDetails.authorizationHeader.authorizationStatus === "Partial Approve" ) ? 
                <PartiallyApprovedStatus>
                    Partially Approved
                </PartiallyApprovedStatus> : (authorizationDetails.authorizationHeader.authorizationStatus === "Void") ? 
                <VoidStatus>
                    Void
                </VoidStatus> : (authorizationDetails.authorizationHeader.authorizationStatus === "Deny") ? 
                <DeniedStatus>
                    Denied
                </DeniedStatus> : 
                <PendingStatus>
                  Pending
                    ...
                    <StartDate>{authorizationDetails.authorizationHeader.startDate}</StartDate></Section>
                    <Section>
                    ...
                    {authorizationDetails.address.addressLine1 === "" && authorizationDetails.phonenumber === null && authorizationDetails.renderingProvider.name === undefined ?
                    null
                    :
                    <Paper>
                      <ServiceProviderTxt>
                      ...
                      {authorizationDetails.address.addressLine1 === "" && authorizationDetails.phonenumber === null ?
                      <span>No Contact Information Available</span>
                      :
                      <ServiceProviderContactInfoWrapper>
                      {authorizationDetails.address.addressLine1 !== "" ?
                      <ExternalSiteLink link={`https://www.google.com/maps?saddr=Current+Location&daddr= ${authorizationDetails.address.addressLine1},
        ${authorizationDetails.address.city}, ${authorizationDetails.address.state}, ${authorizationDetails.address.zip}`} label = "GoogleMaps" target="_blank" styles={{cursor: "pointer"}}>
                        <Address>
                          <span>
                          <LocationIcon src = "/react/images/icn-map-blue.svg">
                            </LocationIcon></span>
                            <AddressTxt>
                            {authorizationDetails.address.addressLine1.concat(' ',authorizationDetails.address.addressLine2,' ',
                        ...
                        <Section onClick={()=> handleSegmentBtn("Mobile Icon")}>
                          ...
                          <PhoneIcon src = "/react/images/icn-call.svg">
                            ...
                            <PhoneNumber>{authorizationDetails.phonenumber}
                            </PhoneNumber>
                          </Section>
                          :
                          null
                    ...
                    <RightContainer>
                    <ServicePapers>
                      {authorizationDetails.authorizationLine.map((service,index) =>(
                    <ServicePaper  first={index === 0 ? true: false} key= {index}>
                    ...
                    <PlaceOfService>{service.serviceOverview.placeOfService}</PlaceOfService>
                    </Section>
                    <Section>
                    <RequestedDateTxt>REQUESTED DATE</RequestedDateTxt>
                    <RequestedDate>{service.serviceOverview.requestedDate}</RequestedDate>
                    </Section>
                    <Section>
                    <ServiceStartDateTxt>START DATE</ServiceStartDateTxt>
                    <ServiceStartDate>{service.serviceOverview.startDate}</ServiceStartDate>
                    </Section>
                    <Section>
                    <ServiceEndDateTxt>END DATE</ServiceEndDateTxt>
                    <ServiceStartDate>{service.serviceOverview.endDate}</ServiceStartDate></Section>
                    <Section>
