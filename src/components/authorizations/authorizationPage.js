...
const AuthorizationPage = () => {
  ...
  const authorizationListLoading = useSelector((state) => state.authorization.loading);
  ...
  const columns = [
    { id: 'authorizationId', selector: 'authorizationId', name: 'AUTHORIZATION #', sortable: true, cell: row => <AuthorizationNbr data-tag="allowRowEvents">{row.authorizationId}</AuthorizationNbr> },
    { id: 'memberName', selector: 'memberName', name: 'MEMBER', sortable: true },
    { id: 'providerName', selector: 'providerName', name: 'PROVIDER', sortable: true },
    { id: 'startDate', selector: 'startDate', name: 'START DATE', sortable: true,
    ...
    {
      id: 'endDate', selector: 'endDate', name: 'END DATE', sortable: true, 
      sortFunction: (rowA, rowB) => {
    ...
    {
      id: 'authorizationStatus', selector: 'authorizationStatus', name: 'STATUS', sortable: true, cell: row => (
        <div onClick={() =>
          history.push({
          ...
          style={{display:'inline-flex'}}>
          {
            row.authorizationStatus === "Approve" ?
              <ProcessedStatus>
                Approved
              </ProcessedStatus> : (row.authorizationStatus === "Pend") ?
                <PendingStatus>
                  Pending
                </PendingStatus> : (row.authorizationStatus === "Partial Deny" || row.authorizationStatus === "Partial Approve") ?
                  <PartiallyApprovedStatus>
                    Partially Approved
                  </PartiallyApprovedStatus> : (row.authorizationStatus === "Void") ?
                    <VoidStatus>
                      Void
                    </VoidStatus> : (row.authorizationStatus === "Deny") ?
                      <DeniedStatus>
                        Denied
                      </DeniedStatus> :
                      <PendingStatus>
  ...
  const Mobilecolumns = [{
    cell: (row) => <Paper onClick={() => history.push({
      ...
      <Content>
        <AuthorizationTxt>AUTHORIZATION </AuthorizationTxt> <Authorization>{row.authorizationId}</Authorization>
      ...
      <Content><DateTxt>START DATE</DateTxt> <DateValue>{row.startDate}</DateValue></Content>
      <Content><EndDateTxt>END DATE</EndDateTxt> <DateValue>{row.endDate}</DateValue></Content>
      <Content><StatusTxt>STATUS</StatusTxt>  {row.authorizationStatus === "Approve" ?
        <MobileProcessedStatus>
          Approved
        </MobileProcessedStatus> : (row.authorizationStatus === "Pend") ?
          <MobilePendingStatus>
            Pending
          </MobilePendingStatus> : (row.authorizationStatus === "Partial Deny" || row.authorizationStatus === "Partial Approve") ?
            <MobilePartiallyApprovedStatus>
              Partially Approved
            </MobilePartiallyApprovedStatus> : (row.authorizationStatus === "Void") ?
              <MobileVoidStatus>
                Void
              </MobileVoidStatus> : (row.authorizationStatus === "Deny") ?
                <MobileDeniedStatus>
                  Denied
                </MobileDeniedStatus> : null
      }
      </Content></Paper>
  }];
      ...
      >
        {authorizationListLoading == false ?
          <Container>
            ...
            <AuthorizationContent>At this time, certain Authorizations may not be up-to-date in your account. If you have any questions, 
              please call the phone number listed on your Member ID card or on the Contact Us tab. </AuthorizationContent>
            <TableContent
              tab="authorization"
          ...
          <Container>
            <AuthorizationText>Authorizations</AuthorizationText>
            <AuthorizationContent>At this time, certain Authorizations may not be up-to-date in your account. If you have any questions, please call the phone number listed on your Member ID card or on the Contact Us tab.</AuthorizationContent>
            <ProgressWrapper>
              <Spinner />
            </ProgressWrapper>
          </Container>
