...
const AddMembershipModal = ({ unmountMe, showModal }) => {
      ...
      if (addMembership.error === "" && addMembership.success === "success") {
        ...
        if (addMembership.error === "no_matches_found") {
          ...
          accountInfo["memberId"] = {
            ...
            error:
              "Make sure that all information matches your enrollment form.",
          };
          ...
          accountInfo["memberId"] = {
            value: accountInfo["memberId"].value,
            error:
              "This Member ID is already attached to an existing account. Please log in to that account to view your plan information. ",
          };
          accountInfo["firstName"] = {
            value: accountInfo["firstName"].value,
            error: " ",
  ...
  const handleClick = () => {
    ...
    if (
      accountInfo["memberId"].error ||
      accountInfo["zipcode"].error ||
  ...
  const getMsg = (msg) => {
    if (msg === "internal_error") {
      return <ToastrText>Uh-Oh! Something went wrong.</ToastrText>;
    } else if (msg === "multiple_customerIds_found") {
      return (
        <ToastrText>
          We are unable to add this membership to your account. Please{" "}
        ...
        <FormModalWrapper visible={showModal}>
          <ModalInnerWrapperCustom>
            <Toastr open={toastr} error={addMembership.error !== ""}>
              <ToastrIconBox error={addMembership.error !== ""}>
                <ToastrIcon
                  alt="toastMsgIcon"
                  src={
                    addMembership.error !== ""
                      ? "/react/images/icn-alert.svg"
                      : "/react/images/valid-check.svg"
                  }
                ></ToastrIcon>
              </ToastrIconBox>
              <ToastrInfo>
                {addMembership.error !== "" ? (
                  getMsg(addMembership.error)
                ) : (
            ...
            <FormModalContent>
              ...
              <div>
                <Header>Please Enter Your Member ID</Header>
                <SubHeader>
                  A Member ID is required to get the most out of your
                  Healthfirst account.
                </SubHeader>
                <FormGrid>
                  <InputWrapper>
                    ...
                    <MemberIdInput>
                      ...
                      <InfoIcon
                        alt=""
                        onMouseOver={() => setShowMemberIdImg(true)}
                        onMouseOut={() => setShowMemberIdImg(false)}
                        src="/react/images/ico-info.png"
                      ></InfoIcon>
                      {showMemberIdImg && (
                        <MemberIdImg
                          alt=""
                          src="/react/images/mem_id.png"
                        ></MemberIdImg>
                      )}
                    </MemberIdInput>
                    {/* {showMemberIdImg */}
        ...
        addMembership.success === "successMedicaid" ? (
          <SuccessModalWrapper>
            <SuccessModalInnerWrapper>
              ...
              />
              <ClockIcon
                src="/react/images/clock.svg"
                color="green"
              ></ClockIcon>
              <SuccessMsg>
                Your membership has been attached, but we still need to verify
                your account.
              </SuccessMsg>
              <SubSuccessMsg>
                Your username and password have been created, but you still need
                to verify your account before you can log in. Call Member
                Services or we can contact you within 72 hours to activate your
                account
              </SubSuccessMsg>
