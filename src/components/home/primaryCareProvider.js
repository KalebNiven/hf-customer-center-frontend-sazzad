...
const PrimaryCareProvider = () => {
  ...
  const pcpDetails =
    pcpHousehold?.data?.hohPlans[customerInfo?.data?.hohPlans[0]?.MemberId];
  ...
  const workdays = (workday) => {
    if (workday !== undefined) {
      ...
      if (todaySchedule && todaySchedule.openAllHours) {
        ...
        todaySchedule &&
        checkIfOpened(todaySchedule.openTime, todaySchedule.closeTime)
      ) {
  ...
  return pcpDetails ? (
    <>
      {" "}
      <GlobalStyle />
      <PcpTxt>Primary Care Provider</PcpTxt>
      <Card className="pcp-coachmark">
        <PcpProvider>
          <ProviderImage alt="" src="/react/images/icon_care_providers.svg" />
          <PcpDetails>
            <ProviderName>
              {pcpDetails.name !== undefined && pcpDetails.name.toLowerCase()}
            </ProviderName>
            <Practice>
              {pcpDetails.practice !== undefined &&
                pcpDetails.practice.toLowerCase()}
            </Practice>
          </PcpDetails>
        </PcpProvider>
        {workdays(pcpDetails.workDays)}
        <ExternalSiteLink
          link={`https://www.google.com/maps?saddr=Current+Location&daddr= ${pcpDetails.address}`}
          label="GoogleMaps"
          target="_blank"
        ...
        >
          <Section>
            ...
            <AddressTxt onClick={() => handleClick("External Link Clicked")}>
              {pcpDetails.address !== undefined &&
                pcpDetails.address.toLowerCase()}
            </AddressTxt>
          </Section>
        </ExternalSiteLink>
        <Section
          onClick={() => handleClick("View or Change Primary Care Provider")}
        >
...
const StatusIcon = styled.p`
  ...
  background-color: ${(props) =>
    props.statusIcn === "Closed" ? `#a8abac` : `#529535`};
`;
