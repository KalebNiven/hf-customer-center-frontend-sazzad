...
const MFAModal = (props) => {
        ...
        if (isComponentMounted && !mfaCode?.loading) {
            if (mfaCode?.error === "" && mfaCode?.data?.data?.verifyCode == 'true') {
                ...
                setNotificationDetails({
                    notificationText: `We sent you ${initialCodeSent ? 'another' : 'a'} code!`,
    ...
    const handleVerify = (data, type) => {
        ...
        if(data.length < 6  || !isNumeric(data)){
            setVerificationCodeInputError('Please ensure that the value entered is 6 digits and try again.');
            return null;
        }
        try {
				...
				return (
                    <div>
                        ...
                        <ContactCardsContainer>
                            <ContactCard key={targetEmail}
                                onClick={() => setSelectedTarget({text: targetEmail, type: 'email'})}
                                status={selectContactError ? 'error' : selectedTarget && selectedTarget?.text === targetEmail ? 'selected' : ""} >
                            ...
                            <ContactCard key={targetPhone}
                                onClick={() => setSelectedTarget({text: targetPhone, type: 'phone'})}
                                status={selectContactError ? 'error' : selectedTarget && selectedTarget?.text === targetPhone ? 'selected' : ""} >
                                <RadioButtonContainer>
                                    {
                                    ...
                                    <ContactValueWrapper>
                                        <ContactValue className="mt-0">
                                            ...
                                            <br /><RatesNoticeText>Message and data rates may apply</RatesNoticeText>
                                        </ContactValue>
                                    </ContactValueWrapper>
                                </RadioButtonContainer>
                            </ContactCard>
                        ...
                        <FormButtonWrapper>
                            { !mfaCode.loading ?
                            <FormButton green={true} onClick={() => handleSendCode(selectedTarget?.text, selectedTarget?.type == 'email' ? 'email' : (selectedTarget?.type == 'phone' ? 'sms' : 'unknown'))}>
                                Send Code
                            </FormButton>
                            :
    ...
    return (
        <div>
            {renderSwitch(step)}
            {renderNotification &&
                        <Toaster toasterTop={'.35in'} unmountMe={() => setRenderNotification(false)} timeout={5000} notificationText={notificationDetails.notificationText} notificationType={notificationDetails.notificationType}/>
            }
        </div>
    );
};
