    ...
    rowCount >= 11 && (
      <PaginationWrapper>
        ...
        <PageList>
          ...
          {visiblePages.length <= 4 &&
            visiblePages.map((page) => {
              return (
                <Button key={page} onClick={() => changePage(page)}>
                  {
                    <NumberButton isCurrent={currentPage === page}>
                      {page}
                    </NumberButton>
                  }
                </Button>
...
const SelectOption = styled.div`
  ...
  &:hover {
    ...
    background-color: ${(props) =>
      props.selected ? "rgba(0, 0, 0, 0.1)" : "#f3f3f3"};
  }
`;
...
const Select = styled.input`
  border-radius: 4px;
  border: ${(props) =>
    props.isOpen ? "solid 1px #003863" : "solid 1px #a8abac"};
  width: 6rem;
