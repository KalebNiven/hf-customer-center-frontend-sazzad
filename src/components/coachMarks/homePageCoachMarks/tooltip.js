...
const Tooltip = ({
  ...
  <TooltipBody
    {...tooltipProps}
    index={index}
    isDesktop={step.stepType === "desktop"}
  >
    {step.stepType !== "desktop" && index === 0 ? null : (
      <CloseButton src="/react/images/icn-close.svg" {...closeProps} />
    )}
    {step.title && <TooltipTitle>{step.title}</TooltipTitle>}
    <TooltipContent>{step.content}</TooltipContent>
    {step.hideControlls ? null : (
      <TooltipFooter>
        ...
        {continuous && <Button {...primaryProps}>Continue</Button>}
      </TooltipFooter>
    )}
  </TooltipBody>
);
