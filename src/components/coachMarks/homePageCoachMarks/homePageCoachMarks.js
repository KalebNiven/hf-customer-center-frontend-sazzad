...
const HomePageCoachMarks = () => {
  ...
  const handleJoyrideCallback = (data) => {
    ...
    if(lifecycle === LIFECYCLE.COMPLETE && action === ACTIONS.PREV && index === 0) {
      setIsStart(true)
      return setRun(false)
    }
    ...
    if((lifecycle === LIFECYCLE.READY) && (action === ACTIONS.NEXT || action === ACTIONS.PREV)) {
    ...
    if([EVENTS.STEP_AFTER, EVENTS.TARGET_NOT_FOUND].includes(type)) {
      ...
      if(action === ACTIONS.PREV) {
        if(steps.type === MOBILE && !steps.list[stepIndex].hideMobileMenu && !menuIsOpen && stepIndex !== 0) {
          setMenuIsOpen(true)
          setRun(false)
          setTimeout(() => {
            setRun(true)
  ...
  const getPassedUsers = () => {
    return localStorage.getItem(HAS_PASSED_TOUR) ? JSON.parse(localStorage.getItem(HAS_PASSED_TOUR)) : [];
  }
...
const handleSegmentBtn  = () => {
  ...
  return (
      <Wrapper className="homePageCoachMarks">
        {hasPassedTour && <RetakeTourButton handleRetakeTour={handleRetakeTour} />}
        { !hasPassedTour && <div>
          <Joyride 
            steps={steps.list}
            ...
            disableOverlayClose
          />
          {isStart && firstName && <FirstTimeScreen visible={isStart} handleStart={handleStart} handleEndTour={handleEndTour} firstName={firstName} steps={steps} screenWidth={screenWidth} />}
          {isEnd && <EndTourScreen goBack={handleGoBack} handleEndTour={handleEndTour} />}
        </div> }
      </Wrapper>
  )
}
