...
const memberForm = ({
        ...
        switch (key) {
          case "memberId":
            if (value.value.length === 0) {
              {
                handleAccountInfo(key, MEMBERID_REQUIRED, value.value);
              }
            } else if (value.value.length < 8) {
              handleAccountInfo(key, INVALID_MEMBERID, value.value);
            }
            break;
          ...
          case "lastName":
            if (value.value.length === 0) {
              handleAccountInfo(key, LASTNAME_REQUIRED, value.value);
            }
            break;

          case "zipcode":
            if (value.value.length === 0) {
              handleAccountInfo(key, ZIPCODE_REQUIRED, value.value);
            } else if (value.value.length < 5) {
              handleAccountInfo(key, ZIPCODE_MINIMUM_CHARACTERS, value.value);
            }
            break;
          ...
          case "dateofBirth":
            if (!moment(value.value, "MM/DD/YYYY").isValid()) {
              ...
              handleAccountInfo(key, MINIMUM_18YEARS_ERROR, value.value);
            }
            break;
          ...
          case "mobile":
            if (
              ...
              handleAccountInfo(key, INVALID_MOBILE_NO, value.value);
            }
            break;
          case "email":
            if (value.value.length === 0) {
          ...
          case "checked":
            if (!value.value) {
              handleAccountInfo(key, TERMS_OF_USE, value.value);
            }
            break;
        }
      }
  ...
  return (
    <>
      <RecaptchaV3
        setV3Response={setGRecaptchaResponse}
        formSubmitted={formSubmissions}
      />
      ...
      <SubHeader>
        Please enter the same information you provided during enrollment.
      </SubHeader>
      ...
      <FormGrid>
        <InputWrapper>
          ...
          <MemberIdInput>
            ...
            {membershipInfo["memberId"].error && (
              <InputErrorMsg>{membershipInfo["memberId"].error}</InputErrorMsg>
            )}
            <InfoIcon
        ...
        <AdditionalInfo>
          <CenterInfo>
            Do you need to make your first payment to activate your coverage?
            &nbsp;
        ...
        <InputWrapper>
          ...
          {membershipInfo["firstName"].error && (
            <InputErrorMsg>{membershipInfo["firstName"].error}</InputErrorMsg>
          )}
        </InputWrapper>
        <InputWrapper>
          ...
          {membershipInfo["lastName"].error && (
            <InputErrorMsg>{membershipInfo["lastName"].error}</InputErrorMsg>
          )}
        </InputWrapper>
        <InputWrapper>
          ...
          {membershipInfo["dateofBirth"].error && (
            <InputErrorMsg>{membershipInfo["dateofBirth"].error}</InputErrorMsg>
          )}
        </InputWrapper>
        <InputWrapper>
          ...
          {membershipInfo["zipcode"].error && (
            <InputErrorMsg>{membershipInfo["zipcode"].error}</InputErrorMsg>
          )}
        </InputWrapper>
        <AdditionalInfo>
          <ItalicFont>
            You will need to verify your information via email or text message.
          </ItalicFont>
        </AdditionalInfo>
        <InputWrapper>
          ...
          {membershipInfo["mobile"].error && (
            <InputErrorMsg>{membershipInfo["mobile"].error}</InputErrorMsg>
          )}
        </InputWrapper>
        <InputWrapper>
          ...
          {membershipInfo["email"].error && (
            <InputErrorMsg>{membershipInfo["email"].error}</InputErrorMsg>
          )}
        </InputWrapper>
        <InputWrapper>
          ...
          {membershipInfo["cEmail"].error && (
            <InputErrorMsg>{membershipInfo["cEmail"].error}</InputErrorMsg>
          )}
        </InputWrapper>
        <CheckBoxWrapper>
          <CheckBox
        ...
        {membershipInfo["checked"].error && (
          <InputErrorMsg>{membershipInfo["checked"].error}</InputErrorMsg>
        )}
      </FormGrid>
      {grVersion === "V2" && (
        <RecaptchaV2
          setV2Response={setGRecaptchaResponseV2}
          formSubmitted={formSubmissions}
        />
      )}
      <FormButtonWrapper>
        {!memberRegister.loading ? (
          <ContinueButton
            variant={variant}
            onClick={() =>
              variant === "primary" ? handleClick(true) : handleClick(false)
            }
          >
            Continue
          </ContinueButton>
