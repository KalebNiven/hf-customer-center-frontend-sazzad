  ...
  const handleCategory = async (id, iconId, title) => {
    ...
    AnalyticsTrack(
      ...
      { data: { ...customerInfo } },
      {
        ...
        description: title,
        destination_url:
          window.location.origin + "/my-health/community-resources/category",
    ...
    if (!enteredZip || enteredZip.length !== 5)
      return setZipInputError("Invalid zip.");
            ...
            {data?.map((row, index) => {
              return (
                <Card
                  ...
                  key={index}
                  space={data.length === index + 1}
                  onClick={() => handleCategory(row.id, row.iconId, row.name)}
                >
                  {iconObject && (
                    <CardImg
                      src={`data:image/svg;base64,${
                        iconObject[origIndex++]?.data?.icon
                      }`}
                    />
                  )}
                  <Text>{row?.name}</Text>
  ...
  return (
    <Wrapper>
      <Container>
        ...
        <SubText>
          The Community Resources are provided for your information only and are
          not covered by your Healthfirst plan.
        </SubText>
        <SubHeader>Find Resources Near</SubHeader>
        <InputOuter hasError={zipInputError.length}>
          <MapImg
            src={`/react/images/${
              zipInputError.length ? "icn-map-red.svg" : "icn-map.svg"
            }`}
          />
          <InputBox
            type="number"
            ...
            hasError={zipInputError.length}
          ></InputBox>
        </InputOuter>
        {zipInputError.length > 0 && (
          <ZipErrorText>{zipInputError}</ZipErrorText>
        )}
        <HorizontalDivider />
...
const InputOuter = styled.div`
  border-radius: 4px;
  border: ${(props) =>
    props.hasError ? "solid 1px #ad122a" : "solid 1px #a8abac"};
