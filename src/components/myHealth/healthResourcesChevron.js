...
const HealthResourcesChevron = ({ currentMemberInfo }) => {
  ...
  const surveyCompleteLocally = (memberId) => {
    ...
    if (
      surveyLocalStatuses &&
      surveyLocalStatuses[memberId] &&
      surveyLocalStatuses[memberId] === "COMPLETE"
    )
      isComplete = true;
    return isComplete;
  };
  ...
  return (
    <>
      {surveyCompleteLocally(currentMemberInfo.memberId) &&
      currentResources[currentMemberInfo.memberId]?.length > 0 ? (
        <ChevronIcon
          src={`/react/images/icn-${
            resoucesVisible.includes(currentMemberInfo.memberId) ? "up" : "down"
          }-arrow.svg`}
          onClick={() =>
            handleResoucesDropdownClick(
              currentMemberInfo.memberId,
              currentMemberInfo.companyCode,
            )
          }
        />
      ) : surveyCompleteLocally(currentMemberInfo.memberId) ? (
