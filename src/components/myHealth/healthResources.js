...
import { getHraStatusesLocalOnly, getMemberAnswers, getAllRecomendedResources } from '../../store/saga/apis'
...
const HealthResources = ({ customerInfo }) => {
    const { setSurveyLocalStatuses, setGeneralResources, setCurrentCompanyCodes, setGeneralResourcesLoader, surveyLocalStatuses, currentResources, setCurrentResources } = useHealthResourcesContext()
    
    const generateUniqueCompanyCodes = (filteredDependents, customerInfo) => {
        let membersCompanyCodes = [...new Set(filteredDependents?.map(dep => dep.companyCode))]
        if(isValidCompanyCode(customerInfo.companyCode, HRA_RESOURCES_COMPANY_CODES) && !membersCompanyCodes.includes(customerInfo.companyCode)) {
    ...
    useEffect(() => {
        ...
        let filteredDependents = customerInfo.dependents?.filter(dep => isValidCompanyCode(dep.companyCode, HRA_RESOURCES_COMPANY_CODES))
        let depMemebrIdsList = filteredDependents?.map(dep => dep.memberId)
    
        // create list of avalable companyCodes
        let membersCompanyCodes = generateUniqueCompanyCodes(filteredDependents, customerInfo)
        setCurrentCompanyCodes([...membersCompanyCodes])
        
        // create membersId list
        let memberIdsList = []
        if(isValidCompanyCode(customerInfo.companyCode, HRA_RESOURCES_COMPANY_CODES)) memberIdsList.push(customerInfo.memberId)
        memberIdsList = [...memberIdsList, ...depMemebrIdsList]
    
        // get local hra statuses by list of memberIds
        getHraStatusesLocalOnly(memberIdsList)
        .then(data => {
            const statuses = {};
            data.forEach(item => {
    ...
    useEffect(() => {
        ...
        const validateRecomendedQuestion = (q, memberAnswers, surveyData) => {
          ...
          const isObese = (weight, height) => {
            ...
            if(Number.isNaN(weightFloat) || Number.isNaN(heightFloat)) return false;
          ...
          memberAnswers[question_id].forEach(ans => {
            ...
            if(ans.value) {
              ...
              if(["v1.2021.0"].includes(surveyData?.questionareId) && ["01", "20"].includes(surveyData?.companyCode) && question_id === "q18") {
            ...
            resources.forEach(resource => {
              resource.assigned_questions.forEach(q => {
                if(validateRecomendedQuestion(q, memberAnswers, surveyData)) {
                  recomendedList.push({ title: resource.title, resource_url: resource.resource_url })
                }
              })
            })
        ...
        const generateGeneralResources = (item) => {
          if(!codeExistList.includes(item[0].companyCode)) {
            generalResourcesList = [...generalResourcesList, ...item[0].general_resources]
        ...
        const setUnfilteredResources = (item) => {
          unfilteredResources[item[0].member_id] = [...item[0].recommended_resources]
        }
  
        const setFilteredResources = (answer) => {
          const { member_id, visited_questions, current_answers, questionare_id, company_code } = answer;
          const answersData = { visited: JSON.parse(visited_questions), memberAnswers: JSON.parse(current_answers)}
          const filteredResources = getRecomendedResources({
            resources: unfilteredResources[member_id],  
            answersData: answersData,
            surveyData: { questionareId: questionare_id, companyCode: company_code }
          })
          allFilteredResources[member_id] = filteredResources
        }
  
        const memberIds = [];
        Object.entries(surveyLocalStatuses).forEach(([key, value]) => value === "COMPLETE" && memberIds.push(key));
  
        getMemberAnswers(memberIds).then((membersAnswers) => {
          
          setGeneralResourcesLoader(true)
          let resourceParams = membersAnswers.map(item => ({ questionareId: item.questionare_id, companyCode: item.company_code, memberId: item.member_id }))
