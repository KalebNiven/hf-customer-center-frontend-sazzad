...
const AnnualHealthAssessment = () => {
  ...
  useEffect(() => {
    ...
    getHraStatuses(customerInfo.memberId, customerInfo.dependents).then(
      (data) => {
        const statuses = {};
        data.forEach((item) => {
  ...
  const getAssessmentLink = (eachDependent) => {
    let hrefLink;
    let hraPlanLowerCamo = {
      companyCode: hraPlan.CompanyNumber,
      memberId: hraPlan.MemberId,
      benefitPackage: hraPlan.BenefitPackage,
    };
    ...
    if (
      ...
      hrefLink = {
        ...
        link: "https://myhfsignaturesurvey.com/healthfirst/hralogin/",
      };
    } else if (
      checkObj.companyCode == "45" &&
      ["PPOM"].some((x) => x == checkObj.benefitPackage)
    ) {
      hrefLink = {
        type: LINK_TYPE.external,
        link: "https://myhfsignaturesurvey.com/healthfirst/hralogin/",
  ...
  const handleSegment = (label, assessmentLink, description) => {
    let assesmentHref =
      assessmentLink.type === LINK_TYPE.cc
        ? window.location.origin + assessmentLink.link
        : assessmentLink.link;
    handleSegmentClick(
      assesmentHref,
      label,
  ...
  const atleastOneMemberValid = (customerInfo) => {
    ...
    const companyCodes = [
      ...new Set([
        ...
        customerInfo?.dependents?.map((dep) => dep.companyCode),
      ]),
    ];
    ...
    <ReactAppWrapper>
      {hraPlan.CompanyNumber !== null && (
        ...
        >
          <Container>
            <GlobalStyle />
            {surveyStatuses ? <Header>Annual Health Assessment</Header> : <></>}
            {surveyStatuses ? (
              <HRAWrapper>
                <CardMain>
                  <CardNav>
                    ...
                    <InlineInnerContainer>
                      <InnerHeader>
                        Take your Annual Health Assessment!
                      </InnerHeader>
                      <Content>
                        Please complete this voluntary Health Questionnaire.
                        This will help us to meet your health care needs. This
                        survey is confidential and will NOT affect your
                        healthcare coverage. If you have any other questions
                        about this Health Questionnaire, please call Member
                        Services.
                      </Content>
                    </InlineInnerContainer>
                  </CardNav>
                  <HorizontalDivider />
                  {atleastOneMemberValid(customerInfo) && (
                    <HealthResources customerInfo={customerInfo} />
                  )}
                  <Main>
                    {surveyStatuses[customerInfo?.memberId] === "COMPLETE" ? (
                      <StatusImg src="/react/images/icn-green-tick.svg" />
                    ) : (
                      <StatusImg src="/react/images/icn-grey-checkmark.svg" />
                    )}
                    <Name>
                  ...
                  {resoucesVisible.includes(customerInfo.memberId) && (
                    <DropDownContent
                      content={
                        currentResources[customerInfo.memberId]
                          ? currentResources[customerInfo.memberId]
                          : []
                      }
                    />
                  )}
                  {customerInfo?.dependents &&
                    customerInfo?.dependents.length > 0 &&
                    customerInfo?.dependents.map((eachDependent, idx) => {
                      return (
                        <Fragment key={idx}>
                          <HorizontalDivider />
                          <Main>
                            {surveyStatuses[eachDependent.memberId] ===
                            "COMPLETE" ? (
                              <StatusImg src="/react/images/icn-green-tick.svg" />
                            ) : (
                              <StatusImg src="/react/images/icn-grey-checkmark.svg" />
                            ...
                            <HealthResourcesChevron
                              currentMemberInfo={eachDependent}
                            />
                          </Main>
                          {resoucesVisible.includes(eachDependent.memberId) && (
                            <DropDownContent
                              content={
                                currentResources[eachDependent.memberId]
                                  ? currentResources[eachDependent.memberId]
                                  : []
                              }
                            />
                ...
                {atleastOneMemberComplete(surveyLocalStatuses) &&
                generalResources?.length > 0 ? (
                  <GeneralHealthResources content={generalResources} />
                ) : generalResourcesLoader ? (
                  <SpinnerWrapper>
                    <Spinner />
                  </SpinnerWrapper>
...
// Check with Igor on GlobalError
...
export default AnnualHealthAssessment;
