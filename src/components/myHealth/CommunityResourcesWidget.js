...
function CommunityResourcesWidget() {
    ...
    const [existingScript, setExistingScript] = useState(document.getElementById(COMMUNITY_RES_WIDGET_SCRIPT_ID));
    ...
    useEffect(() => {
        if(existingScript){
            ...
            loadExternalScript(MIX_REACT_TRAILBLAZER_WIDGET_BASE_URL + '/community-resources-widget.js', COMMUNITY_RES_WIDGET_SCRIPT_ID, 
            () => {
                try {
                    window.CommunityResourcesWidget.mount(mountProps);
                    sessionStorage.setItem("longLoad", false);
                    setExistingScript(document.getElementById(COMMUNITY_RES_WIDGET_SCRIPT_ID));
                } catch (error) {
                    (async () => {
                        try {
                            await logError(error);
