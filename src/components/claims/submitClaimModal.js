...
const useStyles = makeStyles(theme => ({
	root: {
		...
		"& .MuiInputBase-input": {
			...
			fontStyle: "normal",
			'&::placeholder': {
				color: ({ claimInformationError }) => claimInformationError ? '#ad122a' : '#474b55',
				opacity: ({ claimInformationError }) => claimInformationError ? '1' : '0.5',
			}
		},
...
const SubmitClaimModal = ({ unmountMe, showModal }) => {
	...
	const submittedAttestationAgreement = useSelector((state) => state.submitAttestationAgreement);
	...
	const getAddressString = (addressObj) => {
		return [addressObj?.addressLine1, addressObj?.addressLine2, addressObj?.city, addressObj?.county, addressObj?.country].filter(value => value).join(",")
	}
	if (dependents?.length && dependentsAddresses?.length) {
		dependents.forEach((dependent) => {
			const dependentAddress = dependentsAddresses.find((value) => value.memberId === dependent.memberId)
			if (dependentAddress) {
				dependent.address = getAddressString(dependentAddress.info.addresses[0]);
				dependent.phoneNumber = dependentAddress?.info?.addresses[0]?.phones[0] ? dependentAddress?.info?.addresses[0]?.phones[0]?.phoneNumber : '';
			}
		})
	}
	const getHohPlans = () => {
		const hohPlansArr = hohPlans && hohPlans.length > 0 && hohPlans.map(x => ({
			...x,
			...
			phoneNumber: hoh && hoh.length > 0 && hoh.find(y => y.memberId === x.MemberId)?.info?.contact?.contactAddress?.phones[0]?.phoneNumber || null,
			address: getAddressString(hoh && hoh.length > 0 && hoh.find(y => y.memberId === x.MemberId)?.info?.contact?.contactAddress || null)
		}))
		if (hohPlansArr == null) {
			return [];
		}
		...
		switch (step) {
			...
			case 1: {
				...
				for (const [key, value] of Object.entries(claimInformation)) {
					if (['claimNumber', 'accountNumber'].every(x => x !== key)) {
						if (!value.value && key !== 'phoneNumber') {
							...
							else if (!value.value || value.value.toUpperCase() != customerInfo.firstName.toUpperCase() + ' ' + customerInfo.lastName.toUpperCase()) {
								isValid = false
								claimInformationCopy[key] = {
									value: value.value,
				...
				if (isValid) {
					...
					attestationAgreementPayload.targetMemberId = memberDetails.length > 1 ? selectedMember.memberId : memberDetails[0].memberId;
	...
	const handleNext = () => {
		...
		if (isValid) {
			if (step < 2) {
				...
				submitClaimPayload.memberDetails = memberDetails.length > 1 ? selectedMember : memberDetails[0];
	...
	useEffect(() => {
		...
		setLoadingSpinner(loading);
		if (submittedAttestationAgreement?.submittedAttestationAgreement?.data?.message == 'Success' && !submittedAttestationAgreement.loading) {
			setStep(step + 1);
		}
	...
	const renderSubheader = (step) => {
		switch (step) {
			case 0: {
				return dependents.length ? "Select a Member" : "Review Member Information"
			}
			case 1: {
	...
	const updateProgress = (e, x, countStart, countEnd, ind, uploadedFilesLength) => {
		if (claimUploadedFiles.findIndex(({ fileName }) => fileName === x.name) > -1) return null;
		const currentState = {
			status: e.type,
	...
	const fileReaderFunc = (x, ind, handleFilesCustomCallBack) => {
		...
		let countStart, countEnd = 0
		return new Promise((resolve, reject) => {
			reader.onloadstart = function (e) {
				countStart = performance.now();
				const currentState = updateProgress(e, x, countStart, countEnd, ind, uploadedFilesLength);
	...
	const getFileUploadProgressBgColor = (x) => {
		...
		let bgColor = '';
		if (x?.executionTime >= 0 && ['loadstart', 'load', 'progress'].some(y => x?.status === y)) {
			bgColor = '#008bbf';
		}
	...
	const renderSwitch = (step) => {
		...
		switch (step) {
								...
								<MemberCardsContainer>
									{
										memberDetails.map(((member) => (
											<MemberCard key={member.memberId}
												...
												status={selectMemberError ? 'error' : selectedMember && selectedMember.memberId === member.memberId ? 'selected' : ""} >
									...
									{selectMemberError && <ErrorLabel>{selectMemberError}</ErrorLabel>}
								</MemberCardsContainer>
							) :
								<MemberCard>
									<Header className="mt-0">
					...
					<FormGrid>
						<InputWrapper>
							...
							<DropdownSelect
								...
								selected={{ label: claimInformationCopy["reasonForSubmission"]?.value } || { label: 'Select' }}
						...
						<InputWrapper>
							...
							<MuiPickersUtilsProvider utils={DateFnsUtils}>
								<DatePicker
									...
									onChange={(value) => setClaimInformation({
										...claimInformationCopy,
										dateOfService: {
											value: value,
											error: null
							...
							{!claimInformationCopy["dateOfService"].error && <InfoMsg>Only one DOS per submission</InfoMsg>}
							{claimInformationCopy["dateOfService"].error && <InputErrorMsg>{claimInformationCopy["dateOfService"].error}</InputErrorMsg>}
						</InputWrapper>
						<InputWrapper>
							<InputHeader>Provider/Facility Name</InputHeader>
							<Input type="text" placeholder="Enter Provider/Facility Name"
							...
							/>
							{claimInformationCopy["facilityName"].error && <InputErrorMsg>{claimInformationCopy["facilityName"].error}</InputErrorMsg>}
						</InputWrapper>
						<InputWrapper>
							<InputHeader>Provider/Facility Phone Number</InputHeader>
							<PhoneInput
								containerStyle={claimInformationCopy["phoneNumber"]?.error ? { border: 'solid 1px #ad122a', borderRadius: '4px' } : {}}
								country={"us"}
							...
							/>
							{claimInformationCopy["phoneNumber"]?.error && <InputErrorMsg>{claimInformationCopy["phoneNumber"]?.error}</InputErrorMsg>}
						</InputWrapper>
						<InputWrapper>
							<InputHeader>Claim/Bill Amount</InputHeader>
							<Input type="number" placeholder="Enter Claim/Bill Amount" value={claimInformationCopy["claimAmount"].value}
								onChange={(e) => setClaimInformation({
									...claimInformationCopy,
									claimAmount: {
							...
							{claimInformationCopy["claimAmount"].error && <InputErrorMsg>{claimInformationCopy["claimAmount"].error}</InputErrorMsg>}
						</InputWrapper>
						<InputWrapper>
							<InputHeader>Claim Number</InputHeader>
							<Input type="number" placeholder="Enter Claim Number" value={claimInformationCopy["claimNumber"].value}
								onChange={(e) => setClaimInformation({
									...claimInformationCopy,
									claimNumber: {
										value: e.target.value,
							...
							{claimInformationCopy["claimNumber"].error && <InputErrorMsg>{claimInformationCopy["claimNumber"].error}</InputErrorMsg>}
						</InputWrapper>
						<InputWrapper>
							<InputHeader>Account Number</InputHeader>
							<Input type="number" placeholder="Enter Account Number" value={claimInformationCopy["accountNumber"].value}
								onChange={(e) => setClaimInformation({
									...claimInformationCopy,
									accountNumber: {
										value: e.target.value,
							...
							<InfoMsg>If Applicable</InfoMsg>
							{claimInformationCopy["accountNumber"].error && <InputErrorMsg>{claimInformationCopy["accountNumber"].error}</InputErrorMsg>}
						</InputWrapper>
						<AttestationWrapper>
							<AttestationTitle>Member Attestation</AttestationTitle>
							<AttestationBody>
								By signing below, I certify that I have paid the dollar amount listed for the specified services received while a Healthfirst member. I further certify that the documents attached to this form demonstrating proof of payment are accurate, true, and complete, in all respects. I also understand that any Healthfirst determination for reimbursement will be made subject to cost share and coinsurance requirements.<br /><br />
							...
							<InputWrapper>
								...
								<AttestationInput type="text" placeholder="Enter Full Name" value={claimInformationCopy["fullName"].value}
									onChange={(e) => setClaimInformation({
										...claimInformationCopy,
										fullName: {
											value: e.target.value,
								...
								{claimInformationCopy["fullName"].error && <InputErrorMsg>{claimInformationCopy["fullName"].error}</InputErrorMsg>}
							</InputWrapper>
							<AttestationCheckboxWrapper>
								<AttestationCheckboxCustom className="checkbox-container" htmlFor="attestationCheckbox" checked={claimInformationCopy["attestationChecked"].value} onClick={(e) => setClaimInformation({
									...claimInformationCopy,
									attestationChecked: {
										value: !claimInformationCopy["attestationChecked"].value,
										error: null
									}
								})} error={claimInformationCopy["attestationChecked"].error}><p>I certify that all information provided is truthful and authentic.</p>
									<span><AttestationCheckChar checked={claimInformationCopy["attestationChecked"].value} error={claimInformationCopy["attestationChecked"].error}></AttestationCheckChar></span>
			...
			case 2:
				return (
					<FileUploadContent>
						<FileUploadTipHeading>Upload up to 5 of the following:</FileUploadTipHeading>
						<FileUploadListContainer>
							{
								['Itemized Receipt - (Proof of Payment)', 'Provider Bill'].map((x, ind) => {
						...
						{!fileLengthError && <FileUploadTipHeading>Maximum file size:<FileUploadSubTip> 5 Mbs</FileUploadSubTip></FileUploadTipHeading>}
						{fileLengthError && <MaxUploadedFileText>{claimUploadedFiles.length <= 0 ? 'A document is required to continue.' : 'Only 5 files can be submitted.'}</MaxUploadedFileText>}
			...
			default:
				return (
					<DefaultModal>
						...
						<SubHeader>
							{retryCount <= 3 ? `The claim failed to submit. Please try again.` : 'System is currently down. Please contact System Administrator.'}
						...
						<FormButtonWrapper>
							<StyledButton variant={retryCount > 2 ? 'disabled' : "secondary"} onClick={() => {
								setStep(step - 1);
								setRetryCount(0)
							...
							<Spacer></Spacer>
							<StyledButton disabled={retryCount > 2} variant={retryCount > 2 ? 'disabled' : "primary"} onClick={() => {
								...
								{renderSwitch(step)}
								{
									step <= 2 && (
										<FormButtonWrapper>
											...
											<Spacer></Spacer>
											<SubmitButton   variant={fileSizeError ? "disabled" : "primary"} onClick={handleNext} disabled={fileSizeError}>

												{(step === 2 && !loadingSpinner) ? 'Submit' : (step === 2 && loadingSpinner) ? <ProgressSpinner /> : 'Continue'}
											</SubmitButton>
										</FormButtonWrapper>
									)
								}
...
margin-top: 16px;
border-radius: 4px;
border:${props => props.fileLengthError ? '2px solid #ad122a;' : '2px dashed #d8d8d8;'} 
padding:0px;
&:hover{
	border:${props => props.fileLengthError ? '2px solid #ad122a;' : '2px solid #008bbf;'} 
...
const FileUploadProgressBar = styled.div`
  ...
  height: 4px;
  margin-top: 7px;
  background: ${(props) => `linear-gradient(to right ,${props.bgColor} 50%, #d8d8d8 0)`};
...
const datePickerStyles = (border, claimInformationCopy) => {
	return {
		height: 45,
		border: border ? "solid 2px #003863" : claimInformationCopy["dateOfService"].error ? "solid 1px #ad122a" : "solid 1px #a8abac",
