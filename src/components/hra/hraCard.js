...
function HRA() {
  ...
  const memberships = [
    ...customerInfo.data.hohPlans.map((plan) => ({
      companyCode: plan.CompanyNumber,
      benefitPackage: plan.BenefitPackage,
      memberId: plan.MemberId,
    })),
    ...customerInfo.data.dependents.map((dep) => ({
      companyCode: dep.companyCode,
...
function HRACard() {
  const {
    data,
    form: { active, selected, pending, visited },
  ...
  useEffect(() => {
    ...
    getHraStatus(memberId)
      .then((data) => {
        setSurveyStatus({
          status: data.status,
          days_from_completion: data.days_from_completion,
        });
      })
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getHraPartials(memberId).then((data) => {
      if (data.status === "Completed") {
        setSurveySuccess(true);
      }
      if (
        data.status === "In Progress" &&
        Number(surveyStatus.days_from_completion) < 365
      ) {
  ...
  const generateUserResposes = (data, selected, visited) => {
    ...
    visited.forEach((questionId) => {
      const question_type = getQuestionById(
        data.list,
        questionId,
        visited,
        hraMemberInfo,
      ...
      if (question_type === MATRIX) {
        ...
        const matrix_questions = getQuestionById(
          data.list,
          questionId,
          visited,
          hraMemberInfo,
        ...
        matrix_questions.forEach((question) => {
          ...
          const answerIndexs =
            selected[questionId + "-" + question.question_id];
          answerIndexs &&
            answerIndexs.forEach((answerIndex) => {
              const option = {
                ans_id:
                  ...
                  "-" +
                  answerIndex.answer_code,
                is_select:
                  question.question_type === SINGLE_SELECT ? true : false,
                free_text:
                  (answerIndex.answer_value && answerIndex.answer_value) || "",
              };
  ...
  const validateSelection = () => {
    ...
    if (question_type === MATRIX) {
      ...
      for (let i = 0; i < matrix_questions.length; i++) {
        ...
        if (
          !selected[active + "-" + item.question_id] ||
          !selected[active + "-" + item.question_id].length
        ) {
    ...
    if (question_type === FREE_TEXT) {
      const selectionEmpty =
        ...
        selected[active][0].value.trim().length === 0;
  ...
  const isLastQuestion = () => {
    ...
    switch (currentQuestion.question_type) {
      case MATRIX: {
        const matrixRootDeps =
          currentQuestion.question_answers[0].answer_dependencies.length;
      ...
      case FREE_TEXT: {
        const depsLength =
          currentQuestion.question_answers[0] &&
          currentQuestion.question_answers[0].answer_dependencies.length;
        return depsLength === 0;
      }
      case SINGLE_SELECT: {
  ...
  return (
    <HRAWrapper>
      ...
      <CardWrapper visible={visible}>
        {
          <Card>
            {surveyStatus?.status === "COMPLETE" || surveySuccess ? (
              <SurveySuccess />
            ) : surveyStatus?.status === "NOT COMPLETE" &&
              data.list.length > 0 ? (
              <>
                <ProgressBar
                  style={{
                    width:
                      active === "q01"
                        ? `0%`
                        : `${
                            (100 / data.list.length) *
                            Number(getCurrQuestionIndex(data, active))
                          }%`,
                  }}
                />
                <CardQuestion
                ...
                <Controls>
                  <div>
                    {active !== "q01" && (
                      <ChangeStepBtn
                        previous
                        onClick={() => handlePreviousQuestionBtn()}
                      >
                        Previous
                      </ChangeStepBtn>
                    )}
                  </div>
                  <ChangeStepBtn onClick={() => handleNextQuestionBtn()}>
