...
const PlanDetailsBlock = ({ memberSelection }) => {
  ...
  return (
    <MainCard>
      <InnerCard
        isEmpty={
          !coverage.deductibleFamily &&
          !coverage.deductibleIndividual &&
          !coverage.maxOutOfPocketFamily &&
          !coverage.maxOutOfPocketIndividual
        }
      >
        <Title>{planName && toTitleCase(planName)}</Title>
        <Status status={membershipStatus.toUpperCase()}>
          {membershipStatus.toUpperCase()}
        </Status>
        <MemberID>Member ID: {memberId}</MemberID>
        <DateValid>
          {moment(membershipEffectiveDate).format("LL")} -{" "}
          {moment(membershipExpirationDate).format("LL")}
        </DateValid>
              ...
              <DataBlock>
                ...
                {coverage.deductibleFamily && (
                  <CostData>
                    ${coverage.deductibleFamily}{" "}
                    {coverage.deductibleFamily && (
                      <SubCategory width="true">Family</SubCategory>
                    )}
                  </CostData>
                )}
                {coverage.deductibleIndividual && (
                  <CostData>
                    ${coverage.deductibleIndividual}{" "}
                    <SubCategory width="true">Individual</SubCategory>
                  </CostData>
                )}
              </DataBlock>
            </Row>
