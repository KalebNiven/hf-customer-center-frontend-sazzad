...
const DependentBlock = ({ memberSelection, setMemberSelection, halfWidth }) => {
    ...
    const handleMemberSelection = (memberInfo) => {
      const { planName, membershipStatus, membershipEffectiveDate, membershipExpirationDate, companyCode, benefitPackage, firstName, lastName, value } = memberInfo;
      setMemberSelection({
        ...memberSelection,
        memberId: value,
        planName,
    ...
    return (
        <>
            { members && members.length > 1 && <MemberDropDownSelectWrapper halfWidth>
