...
const MailIdCard = (props) => {
  ...
  useEffect(() => {
    if (
      ...
      (typeof mailMemberIDCardStatus.data !== "undefined" &&
      mailMemberIDCardStatus.data !== null &&
      mailMemberIDCardStatus.data.length != 0
        ? true
        : false)
    ) {
      setLatestStatusRecord(
        getLatestStatus(
          mailMemberIDCardStatus.data[1],
          mailMemberIDCardStatus.data[0].status,
          mailMemberIDCardStatus.data[0][mailMemberIDCardStatus.data[1]],
    ...
    switch (status) {
      case "Request initiated":
        return {
          title: "Processing",
          description:
            "Your New Member ID Card request is currently being processed.",
          ...
          currAddr:
            newAddress?.newAddressLn != null
              ? standardizeAddress(newAddress, "newAddress")
              : standardizeAddress(mailingAddress, "mailingAddress"),
          prevAddr:
            newAddress?.newAddressLn != null
              ? standardizeAddress(mailingAddress, "mailingAddress")
              : null,
      ...
      case "In progress":
        return {
          ...
          description:
            "Your New Member ID Card is getting ready to ship to your address.",
          lastUpdateDate: formatDate(statusDate),
          carrier: carrier,
  ...
  return (
    <Container>
          ...
          {latestStatusRecord != null ? (
            <>
              ...
              <MailIdCardContainer>
                <Card>
                  ...
                  <StatusDate>
                    Last Updated {latestStatusRecord["lastUpdateDate"]}
                  </StatusDate>
                  <StatusDescription>
                    {latestStatusRecord["description"]}
                  </StatusDescription>
                  {latestStatusRecord["trackingCodeURL"] != null &&
                  latestStatusRecord["trackingCode"] != null ? (
                    <TrackingContainer>
                      ...
                      <TrackingVerbiage>
                        ...
                        >
                          {latestStatusRecord["trackingCode"]}
                        </TrackingLink>
                      </TrackingVerbiage>
                    </TrackingContainer>
                  ) : null}
                  <DeliveryAddressesContainer>
                    {latestStatusRecord["currAddr"] != null ? (
                      <DeliveryAddressContainer>
                        ...
                        <DeliveryAddress>
                          {latestStatusRecord["currAddr"]["addr1"] +
                            " " +
                            latestStatusRecord["currAddr"]["addr2"]}
                        </DeliveryAddress>
                        <DeliveryAddress>
                          {latestStatusRecord["currAddr"]["city"] +
                            ", " +
                            latestStatusRecord["currAddr"]["state"] +
                            " " +
                            latestStatusRecord["currAddr"]["zip"]}
                        </DeliveryAddress>
                      </DeliveryAddressContainer>
                    ) : null}
                    {latestStatusRecord["prevAddr"] != null ? (
                      <DeliveryAddressContainer>
                        ...
                        <DeliveryAddress>
                          {latestStatusRecord["prevAddr"]["addr1"] +
                            " " +
                            latestStatusRecord["prevAddr"]["addr2"]}
                        </DeliveryAddress>
                        <DeliveryAddress>
                          {latestStatusRecord["prevAddr"]["city"] +
                            ", " +
                            latestStatusRecord["prevAddr"]["state"] +
                            " " +
                            latestStatusRecord["prevAddr"]["zip"]}
                        </DeliveryAddress>
                      </DeliveryAddressContainer>
                    ) : null}
                  </DeliveryAddressesContainer>
            ...
            <>
              ...
              <Description className="no-print">
                Order a new Member ID Card and have it send directly to your
                home address.
              </Description>
              <MailIdCardContainer>
                <MailIdCardButton
                  ...
                  disableBtn={
                    typeof mailMemberIDCardStatus.data !== "undefined" &&
                    mailMemberIDCardStatus.data !== null &&
                    mailMemberIDCardStatus.data[0] !== null
                      ? true
                      : false
                  }
                  memberId={props?.memberSelection?.memberId}
                />
                {(
                  typeof mailMemberIDCardStatus.data !== "undefined" &&
                  mailMemberIDCardStatus.data !== null &&
                  ...
                  <RecentRequestText className="no-print">
                    A physical Member ID Card was recently requested. A new
                    request can be made after 14 days.
                  </RecentRequestText>
                ) : null}
              </MailIdCardContainer>
            </>
      ...
      <MailMemberIDCardForm
        showForm={renderIdCardForm}
        member={props?.memberSelection}
        customerDemographicsInfo={customerDemographicsInfo}
        unmountMe={() => setRenderIdCardForm(false)}
      />
    </Container>
  );
};
