...
const MemberIdCardWidget = () => {
  ...
  const memberId = location?.state?.dependentMemberId
    ? location?.state?.dependentMemberId
    : customerInfo?.hohPlans[0]?.MemberId;
  const {
    digitalSurveyWidget,
  ...
  useEffect(() => {
    if (existingScript) {
      ...
      loadExternalScript(
        MIX_REACT_TRAILBLAZER_WIDGET_BASE_URL + "/id-card-widget.js",
        MEMBER_ID_CARD_WIDGET_SCRIPT_ID,
        () => {
          try {
            window.IDCardWidget.mount(mountProps);
