...
const MemberIDCardPage = (props) => {
  ...
  useEffect(() => {
    //Do we need this... or can we just listen on location change in the above?
    if (location.state) {
      ...
      >
        {renderNotification && (
          <Toaster
            ...
            timeout={5000}
            notificationText={"An example notification"}
            notificationType={"success"}
          />
        )}
        <IDCardTitle className="no-print">Member ID Card</IDCardTitle>
        <IDCardText className="no-print">
          This section contains all necessary information for your membership
          that may not exist on your physical ID card.
        </IDCardText>
        {memberSelection.memberId != null &&
          memberSelection.memberId != undefined && (
            <>
