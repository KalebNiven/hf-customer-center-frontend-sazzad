...
import {
  requestCustomerInfo,
  requestContactEmailInfo,
  requestContactPhoneInfo,
...
import {
  verifyContactInfo,
  resendCodeContactInfo,
  verifyPhoneContactInfo,
...
const ContactInformation = () => {
  ...
  useEffect(() => {
    ...
    if (
      !loading &&
      error === "" &&
      (submitEmailDetails || submitPhoneDetails)
    ) {
    ...
    if (error !== "") {
      emailAddressEditing
        ? setEmailError(error)
        : setPhoneNumber(() => ({ ...phoneNumber, error: error }));
    }
  }, [submitContactInfoPayload]);
  useEffect(() => {
  ...
  const validatePhone = () => {
    if (!phoneNumber.value) {
      ...
      setPhoneNumber((phoneNumber) => ({
        ...phoneNumber,
        error: "Please use 10-digit number format",
      }));
    } else if (
      customerInfo?.mobilePhone === phoneNumber.value &&
      !phoneVerifying
    ) {
      setPhoneNumber((phoneNumber) => ({
        ...phoneNumber,
  ...
  const verifyEmailLink = async () => {
    ...
    verifyEmailContactInfo(payload, true)
      .then((res) => {
        setVerificationModalData(emailAddress);
        setCurrentModal("email");
      })
  ...
  const verifyPhoneLink = async () => {
    ...
    verifyPhoneContactInfo(payload, true)
      .then((res) => {
        setVerificationModalData(phoneNumberHashed);
        setCurrentModal("sms");
      })
  ...
  const resendVerificationCode = async () => {
    ...
    const responseData = resendCodeContactInfo(payload, customerInfo.csrf)
      .then((res) => {
      ...
      <FormModalWrapper visible={verificationModalData}>
        <ModalInnerWrapperCustom>
          <FormModalContent>
            ...
            />
            <>
              <HeaderCustom>Enter Verification Code</HeaderCustom>
              <SubHeaderCustom>
                A verification code has been sent to:{" "}
                <SubHeaderBold>{verificationModalData}</SubHeaderBold>
              </SubHeaderCustom>
              <Label>Verification Code</Label>
              <TextBox error={postVerifyError.error}>
                <InputBox
                  error={postVerifyError.error}
                  type="number"
              ...
              <Tip emailErrorCheck={postVerifyError?.error}>
                {postVerifyError?.message} &nbsp;
              </Tip>

              <SendCodeText>
                Didn’t get the code?{" "}
                <LinkText
                  resendCodeState={resendCodeState}
                  onClick={() => {
                    setResendCodeState(false);
              ...
              <ContactUsext>
                If you are still unable to verify,{" "}
                <LinkText
                  resendCodeState={true}
                  onClick={() =>
                    window.open("https://healthfirst.org/contact", "_blank")
                  }
                >
                  please contact us
                </LinkText>
              ...
              <FormButtonWrapper>
                ...
                <Spacer></Spacer>
                <StyledButtonCustomVerify
                  variant={verificationCode.length < 6 ? "disabled" : "primary"}
                ...
                >
                  {loadingSpinner ? (
                    <ProgressWrapperCustom>
                      <ProgressSpinnerCustomVerify />
                    </ProgressWrapperCustom>
                  ) : (
                    "Verify and Save"
                  )}
  ...
  return (
    <>
      ...
      <SubHeader>
        Control what information and methods Healthfirst will use to contact
        you.
      </SubHeader>
      <Card>
        ...
        {emailAddressEditing ? (
          <>
            ...
            <TextBox error={emailErrorCheck}>
              ...
              <UserImg
                alt="sd"
                src="/react/images/Close.svg"
                onClick={() => setEmailAddress("")}
              />
            </TextBox>
            <Tip emailErrorCheck={emailErrorCheck}>
              {emailErrorCheck
                ? emailError
                : "Your email address must be verified before the change takes effect."}
            </Tip>
            <FormButtonWrapper>
              <StyledButtonCustom
                onClick={() => {
              ...
              <Spacer></Spacer>
              <StyledButtonCustomSave
                variant={"primary"}
                onClick={validateEmail}
              >
                {loadingSpinnerSave ? (
                  <ProgressWrapperCustomSave>
            ...
            <EmailReadFlexColVerify>
              ...
              <RowBlockVerify>
                <UserImg
                  ...
                  src={
                    emailStatus
                      ? "/react/images/Checkmark-Circle.svg"
                      : "/react/images/Unverified.svg"
                  }
                />
                <VerfiedLabel alt="" emailStatus={emailStatus}>
      ...
      <Card>
        ...
        <Text>
          In case we need to contact you via phone or text, please make sure
          your number(s) are up to date.
        </Text>
        {phoneEditing ? (
          <>
            ...
            <RowBlockCustom>
              ...
              <ColBlockPhoneNumber>
                ...
                <TextBox error={phoneNumber.error}>
                  <InputBox
                    value={phoneNumberHashed}
                    error={phoneNumber.error}
                    onChange={(e) => {
                      let currentValue = e.target.value.replaceAll("-", "");

                      if (currentValue && !currentValue.match(/^[0-9]+$/))
                        return;
                ...
                <Tip emailErrorCheck={phoneNumber.error}>
                  {phoneNumber.error}
                </Tip>
              </ColBlockPhoneNumber>
            </RowBlockCustom>
          ...
          <PhoneReadFlex>
            ...
            <PhoneReadFlexColVerify>
              ...
              <PhoneRowBlockVerify phoneStatus={phoneStatus}>
                ...
                />
                <VerfiedLabel alt="" phoneStatus={phoneStatus}>
                ...
                <PhoneRowBlockVerify>
                  {phoneVerifying ? (
                    ...
                    <>
                      <UserImgVerifyCircle
                        alt=""
                        src={"/react/images/Verify-circle.svg"}
                      />
                      <VerifyLink
                        onClick={() => {
                          verifyPhoneLink();
...
const VerfiedLabel = styled.span`
  color: ${({ emailStatus, phoneStatus }) =>
    emailStatus || phoneStatus ? "#529535" : "#757575"};
  font-size: 14px;
  line-height: 1.33;
  font-weight: bold;
`;
...
const ToastrIconBox = styled.div`
  ...
  padding: 0px 8px;
  height: 64px;
  background-color: ${({ toastrState }) =>
    toastrState === "Success" ? "#36502a" : "#ad122a"};
  align-items: center;
...
const Toastr = styled.div`
  ...
  position: fixed;
  background-color: ${({ toastrState }) =>
    toastrState === "Success" ? "#3e7128" : "#ad122a"};
  top: 130px;
  left: 50%;
...
const LinkText = styled.span`
  ...
  cursor: ${({ resendCodeState }) =>
    resendCodeState ? "pointer" : "not-allowed"};
  font-weight: 900;
`;
