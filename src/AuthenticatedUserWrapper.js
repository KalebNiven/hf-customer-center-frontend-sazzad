...
import {
  AnalyticsIdentifyNonMember,
  AnalyticsIdentifyMember,
...
const AuthenticatedUserWrapper = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (
      accountStatus === "NON-MEMBER" &&
      !sessionStorage.getItem("skipAddMembership")
    ) {
  ...
  useEffect(() => {
    if (
      !sessionStorage.getItem("visitedPrefCenterSync") &&
      (preferenceCenterInfo?.data?.email?.is_different ||
        preferenceCenterInfo?.data?.phones?.is_different)
    ) {
    ...
    <>
      <ErrorBoundary FallbackComponent={UnrecoverableErrorAuthenticated}>
        {customerInfo.loading == false ? (
          customerInfo.error === "" ? (
            wantsMedicare ? (
              (window.location.href = MIX_REACT_APP_MPSR_LOGIN_URL)
            ) : (
              <FeatureFactory
                splitKey={MIX_SPLITIO_KEY}
                options={featureFlagOptions}
                uniqueId={customerId ? customerId : uuid}
                trafficType="user"
              >
                <AppContextProvider>
