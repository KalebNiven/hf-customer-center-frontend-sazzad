export const getQuestionById = (dataList, id, visited, customerInfo) => {
    function transformQuestionIdString(questionIdString, accumulator) {
        ...
        const qusetionIdNum = Number(questionIdString.slice(sliceCount)) + accumulator;
        const transformedQuestionIdString = `q${qusetionIdNum < 10 ? 0 : ''}${qusetionIdNum}`
    ...
    function isNotValidQuestion(question) {
        ...
        gender = (gender === "F" || gender === "female") ? "female" : (gender === "M" || gender === "male" )? "male" : "all";
        if(question.question_gender !== "all" && question.question_gender !== gender) isNotValid = true;
    ...
    let validQuestion = allMatchedQuestions.find(question => isNotValidQuestion(question) === false)
    ...
    if(isNotValidQuestion(validQuestion) === true) {
        validQuestion = getQuestionById(dataList, validQuestion.question_id, visited, customerInfo)
    }
