...
const PreferredContactInfo = (props) => {
  ...
  useEffect(() => {
    if (isComponentMounted && !submitPreferredContactInfoResponse?.loading) {
      if (
        submitPreferredContactInfoResponse?.error === "" &&
        submitPreferredContactInfoResponse?.data?.data?.errors.length == 0
      ) {
        switch (step) {
          case "Email":
            setStep("Phone");
    ...
    if (step == "Email" && preferenceCenterInfo?.data?.phones?.is_different) {
      ...
      formData = {
        ...
        secondaryPhoneType:
          ...
          preferenceCenterInfo?.data?.phones?.pm_secondary_type !== ""
            ? preferenceCenterInfo.data.phones.pm_secondary_type
            : "mobile",
    ...
    switch (type) {
      case "phone":
        if (preferenceCenterInfo?.data?.phones !== undefined) {
          if (
            preferenceCenterInfo?.data?.phones?.okta !== undefined &&
            preferenceCenterInfo?.data?.phones?.okta != ""
          ) {
            contactsArr.push({
              number: preferenceCenterInfo.data.phones.okta,
              type: "mobile",
              sourceType: "okta",
            });
          }
          if (
            ...
            contactsArr.push({
              number: preferenceCenterInfo.data.phones.pm_primary,
              type: preferenceCenterInfo.data.phones.pm_primary_type,
              sourceType: "pm",
            });
          }
          if (
            ...
            contactsArr.push({
              number: preferenceCenterInfo.data.phones.pm_secondary,
              type: preferenceCenterInfo.data.phones.pm_secondary_type,
              sourceType: "pm",
            });
          }
        }
      ...
      case "email":
        if (
          preferenceCenterInfo?.data?.email?.okta !== undefined &&
          preferenceCenterInfo?.data?.email?.okta != ""
        ) {
          contactsArr.push({
            email: preferenceCenterInfo.data.email.okta,
            sourceType: "okta",
          });
        }
        if (
          ...
          contactsArr.push({
            email: preferenceCenterInfo.data.email.pm,
            sourceType: "pm",
          });
        }
        return contactsArr;
    ...
    switch (step) {
      case "Phone":
        return (
          <div>
            <Header>
              Our records show multiple phone numbers associated with your
              account.
            </Header>
            <SubHeader>
              Please select the mobile phone number we should associate with
              your account.
            </SubHeader>
            <SelectText>Select your primary mobile number</SelectText>
            {!preferenceCenterInfo.loading ? (
              getContacts("phone").length >= 0 ? (
                <ContactCardsContainer>
                  {getContacts("phone").map((contact) => (
                    <ContactCard
                      key={contact.phone + contact.sourceType}
                  ...
                  {/*
                                ...
                                */}
                  {selectContactError && (
                    <ErrorLabel>{selectContactError}</ErrorLabel>
                  )}
                </ContactCardsContainer>
              ) : null
            ) : (
            ...
            <BottomText>
              Your contact information can always be changed within your Account
              Settings.
            </BottomText>
            <FormButtonWrapper>
      ...
      case "Email":
        return (
          <div>
            <Header>
              Our records show multiple email addresses associated with your
              account.
            </Header>
            <SubHeader>
              Please select the email address we should associate with your
              account.
            </SubHeader>
            <SelectText>Select your preferred email address</SelectText>
            {!preferenceCenterInfo.loading ? (
              getContacts("email").length >= 0 ? (
                <ContactCardsContainer>
                  {getContacts("email").map((contact) => (
                    <ContactCard
                      key={contact.email + contact.sourceType}
      ...
      case "MFA":
        return (
          <MFAModal
            mfaStatus={(status) => handleMFAStatus(status)}
            targetEmail={selectedEmail?.email}
            targetPhone={selectedPhone?.number}
          />
        );
      case "Success":
        return (
